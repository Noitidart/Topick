...
var gCommScope = {};
Services.scriptloader.loadSubScript('chrome://topick/content/resources/scripts/comm/Comm.js', gCommScope);
...
var pageLoader = {
	...
	matches: function(aHREF, aLocation) {
		...
		if (href_lower.startsWith('about:topick')) {
			return MATCH_APP;
		}
	},
	...
	error: function(aContentWindow, aDocURI) {
		...
		if (aContentWindow.location.href.startsWith('about:topick') && Date.now() - gLastReloadDueToError > 100) {
			gLastReloadDueToError = Date.now();
			console.warn('it is about:topick, this hpapens if the connection to load about:topick was established before the about page was setup. which happens on gBrowser.loadOneTab(about:topick). so load it again, href:', aContentWindow.location.href);
...
function aboutRedirectorizer(aURI) {
	...
	if (uripath_lower == 'topick') {
		return core.addon.path.pages + 'app_options.xhtml';
	} else {
		return 'data:text/plain,invalid topick page "' + uripath_lower + '"';
	}
}
...
function init() {
	...
	callInMainworker('fetchCore', undefined, function(aArg, aComm) {
		...
		//
		try {
			gAppAboutFactory = registerAbout('topick', 'topick options page', '{e631b070-6c6d-11e6-bdf4-0800200c9a66}', aboutRedirectorizer);
